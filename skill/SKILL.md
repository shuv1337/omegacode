...
- **pi** (defaults to the **shuvpi** fork binary; ≥ 0.79.1 JSONL surface) — spawn-per-call subprocess backend. `model` is an open pi/shuvpi model reference (e.g. `"openrouter/moonshotai/kimi-k2.6"`). Override the binary with `PI_BIN`. **Requires `sandbox: "danger-full-access"`** — tool allowlists are not OS confinement, so `read-only`/`workspace-write` are rejected. `effort` maps onto pi's full thinking range (`none`→off … `max`→xhigh); `maxTurns` is rejected. Structured output via a silent tool-less extraction turn.
